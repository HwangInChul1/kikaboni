...
// 첨부파일 목록 조회
	$.getJSON( `${ctxPath}/board/getAttachList`, {bno : bnoValue},
	function(attachList){
	...
	$(attachList).each(function(i,e){
		...
		if(e.fileType){ // 이미지 파일이면 섬네일 표시
			...
			fileList += `
				<div class="thumnail d-inline-block mr-3">
					<img alt="" src="${ctxPath}/files/commend/display?fileName=${encodingFilePath}">	
				</div>`
		} else {
			fileList += `
		...
		if(e.fileType){ // 이미지 파일일떄
			...
			fileList += `<a href="${ctxPath}/files/commend/download?fileName=${fileName}" class="download">다운로드</a>` 
		}
			fileList += `</div></li>`
	});
	...
	$('.uploadResultDiv ul').on('click','.showImage', function(e){
		...
		let imgSrc = `${ctxPath}/files/commend/display?fileName=${filePath}`
