...
// 파일 업로드 결과를 화면에 표시
			...
			`<li class="list-group-item" data-uuid="${e.uuid}">
				...
				if(e.fileType){ // 이미지 파일일 때
					...
					`<div class="d-inline-block mr-4 mb-4">
						<img alt="첨부파일" src="${ctxPath}/files/talk/display?fileName=${encodingFilePath}">
					</div>`
				} else { // 이미지 파일이 아닐 때 
					fileList += 
	...
	// 첨부파일 목록 불러오기
	$.getJSON(
		`${ctxPath}/board/talkAttachList`, {bno : bnoValue}, 
		function(attachList){
	...
	$('input[type="file"]').change(function(){
		...
		for(let f of files){
			formData.append('talkuploadFile',f);
		}
	
		$.ajax({
			type : 'post',
			url : `${ctxPath}/files/talkupload`,
			processData : false,
			contentType : false,
	...
	$('.uploadResultDiv ul').on('click','.delete',function(e){
		...
		$.ajax({
			type : 'post',
			url : `${ctxPath}/files/talkDeleteFile`,
			data : targetFile,
	...
	$('.uploadResultDiv ul').on('change','.toBeDelFile',function(e){
		...
		if($(this).is(':checked')){ // 체크
			$.ajax({
				type : 'get',
				url : `${ctxPath}/board/talkAttachFileInfo`,
				data : {uuid : uuidValue},
