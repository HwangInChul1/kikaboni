...
// 파일 업로드 결과를 화면에 표시
			...
			`<li class="list-group-item" data-uuid="${e.uuid}">
				...
				if(e.fileType){ // 이미지 파일일 때
					...
					`<div class="d-inline-block mr-4 mb-4">
						<img alt="첨부파일" src="${ctxPath}/files/menu/display?fileName=${encodingFilePath}">
					</div>`
				} else { // 이미지 파일이 아닐 때 
					fileList += 
	...
	// 첨부파일 목록 불러오기
	$.getJSON(
		`${ctxPath}/board/menuAttachList`, {bno : bnoValue}, 
		function(attachList){
			...
			if(e.fileType){ // 이미지 파일일떄
				...
				fileList += `<a href="${ctxPath}/files/menu/download?fileName="${fileName}">다운로드</a>`	
			}
			fileList += `
	...
	$('.uploadResultDiv ul').on('click','.showImage',function(e){
		...
		let imgSrc = `${ctxPath}/files/menu/display?fileName=${filePath}` // 원본이미지를 보여주기 위한 컨트롤러의 경로를 값으로 저장
	...
	$('input[type="file"]').change(function(){
		...
		for(let f of files){
			formData.append('menuUploadFile',f);
		}
	
		$.ajax({
			type : 'post',
			url : `${ctxPath}/files/menuUpload`,
			processData : false,
			contentType : false,
	...
	$('.uploadResultDiv ul').on('click','.delete',function(e){
		...
		$.ajax({
			type : 'post',
			url : `${ctxPath}/files/menuDeleteFile`,
			data : targetFile,
	...
	$('.uploadResultDiv ul').on('change','.toBeDelFile',function(e){
		...
		if($(this).is(':checked')){ // 체크
			$.ajax({
				type : 'get',
				url : `${ctxPath}/board/menuAttachFileInfo`,
				data : {uuid : uuidValue},
